...
class IndexController extends RestController {
    ...
    public  function recordingExport($time=0){
        ...
        $lmap['_string'] = " ol.operate in(-1,1,2,3,4,5,6,18,23,24,33,34) or (ol.operate in(20,21) and ol.status = 1)";
        $result = $model->alias('ol')->field('b.user_name,um.mobile,ol.plate_no,ol.uid,ol.pic1,ol.pic2,ol.gis_lng,ol.gis_lat,ol.operate,ol.car_status,ol.time,ol.source,ol.car_rule,ol.before_battery,ol.remark,ol.desc')
            ->join(' (SELECT job_type,user_id,user_name,corporation_id from  baojia_mebike.repair_member GROUP BY user_id)  b on ol.uid = b.user_id','left')
            ->join('corporation c on c.id=b.corporation_id','left')
        ...
        if($result){
            ...
            foreach ($result as $key=>&$val){
                if($val['operate'] == 1){
                    ...
                    $val['operate'] = '完成小修';
                    $val['pic1']    = $val['pic2']?"http://pic.baojia.com/".$val['pic2']:"";
        ...
        $title = array('车牌号','操作人','手机号','操作任务','操作时间','换电前电量','换电后电量','备注信息','附件');
        $file = "员工工作记录";
