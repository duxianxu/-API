...
class GpsModel extends Model{
    ...
    public function gpsStatusData($imei,$latitude,$longitude){
        ...
        try {
            if (!empty($imei)) {
                $map["imei"] = $imei;
                $info = M("gps_status", null, $box_config)->where($map)->find();
                $data['latitude'] = $latitude;
                $data['longitude'] = $longitude;
                if ($info) {
                    \Think\Log::write("位置校正更新坐标，更新前坐标:" . $info["longitude"] . "，" .$info["latitude"]."，将要更新坐标:". $longitude . "，" .$latitude, "INFO");
                    if($info["longitude"]==$longitude&&$info["latitude"]==$latitude){
                        return -100;
                    }
                    $result = M("gps_status", null, $box_config)->where($map)->save($data);
					if($result){
						$Details = new \Api\Model\DetailsModel();
						$res_arr = $Details->getXiaomiPosition($imei);
						if($res_arr){
							$res_arr["latitude"] = $latitude;
							$res_arr["longitude"] = $longitude;
							...
							$Redis->set($key,$res_arr);
						}
					}
					// $result = $this->reportedGps($imei,$latitude,$longitude);
                }
                \Think\Log::write("位置校正更新坐标，参数：imei:" . $imei . json_encode($data) . "，结果：" . $result . "，更新前：" . json_encode($info), "INFO");
                return $result > 0;
            } else {
                return 0;
            }
        }catch (Exception $exception){
            \Think\Log::write("位置校正更新坐标失败:".$exception, "ERROR");
            return 0;
        }
    }
	
	public  function   reportedGps($imei='',$latitude='',$longitude=''){
        ...
        $data["longitude"] = $longitude;
        $data["gpsType"]   = 1;
        $url = "10.1.11.97:8080/distribute/app/save/gps";
        $result = $this->VoltagePost($data,$url);
        
        \Think\Log::write("第三方位置校正更新坐标，参数：imei:" . $imei . json_encode($data) . "，结果：" .  json_encode($result), "INFO");
        if($result["code"] == 200){
            return 1;
        }else{
            return 0;
